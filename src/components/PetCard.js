...
const PetCard = (props) => {
  const { id, name, species, about, location, selectDetailsCallback, removePetCallback } = props;
  return (
    <div className="card pet-card">
      ...
      { speciesEmoji(species) } {id} - {name}
        <button
          ...
          onClick={ () => {selectDetailsCallback(props.id)} }
          >
            Select
        </button>
        <button
          ...
          aria-label="Close"
          onClick={ () => {removePetCallback(props.id)} }
        >
          Close
        </button>
      </section>
...
PetCard.propTypes = {
  ...
  selectDetailsCallback: PropTypes.func,
  removePetCallback: PropTypes.func,
}
